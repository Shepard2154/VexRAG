# Scan configs (NQ + ChromaDB)

Start the target first: `python3 nq_rag.py` (from the parent directory). First run builds the Chroma index from NQ (see `config.json` `nq_dataset.limit`).
...
| **`ollama-smoke-native-poisoner.yaml`** | `llama3:8b`, `all-MiniLM-L6-v2` (embed) | **Native Chroma** → `../chroma_data/` | 2 hijack + 1 poisoned |
...
- **`ollama-smoke-native-poisoner.yaml`** — VexRAG writes adversarial vectors directly into the Chroma collection (`scan.corpus_poisoning.backend: chroma`). No poison files are required; the target reads poison hits from the same DB path/collection as in `config.smoke.json`.
