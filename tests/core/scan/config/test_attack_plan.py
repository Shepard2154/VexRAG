...
class TestAttackPlan:
    ...
    def test_parse_rejects_legacy_attack_key(self) -> None:
        reg = _stub_registry("alpha")
    ...
    def test_materialize_merges_scan_and_evaluation(self) -> None:
        step = AttackStepSpec(
            attack_id="alpha",
            params={"k": 1},
    ...
    def test_resolve_generate_cases_auto_single(self) -> None:
        reg = _stub_registry("alpha")
        cfg = {"attacks": [{"id": "alpha", "params": {}}]}
        assert resolve_generate_cases_attack_id(cfg, reg, explicit=None) == "alpha"

    def test_resolve_generate_cases_auto_multi_requires_explicit(self) -> None:
    ...
    def test_materialize_config_for_attack_id_picks_step(self) -> None:
        reg = _stub_registry("alpha", "beta")
        cfg = {
            "attacks": [
                {"id": "alpha", "params": {"a": 1}},
